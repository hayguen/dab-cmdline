...
//	We do need an option to specify the filename
  ...
  try {
#ifdef HAVE_SDRPLAY
    theDevice = new sdrplayHandler(frequency, ppmCorrection, GRdB, lnaState,
                                   autogain, 0, 0);
#elif HAVE_AIRSPY
    theDevice = new airspyHandler(frequency, ppmCorrection, deviceGain);
#elif HAVE_RTLSDR
    theDevice =
        new rtlsdrHandler(frequency, ppmCorrection, deviceGain, autogain);
#elif HAVE_HACKRF
...
#elif HAVE_RTL_TCP
  theDevice = new rtl_tcp_client(hostname, basePort, frequency, deviceGain,
  ...
  while (!timeSynced.load() && (--timeSyncTime >= 0)) sleep(1);
...
void printOptions(void) {
  std::cerr << "                          dab-cmdline options are\n\
