...
/*
 */
audioSink::audioSink(int16_t latency, std::string soundChannel, bool *err)
    : audioBase() {
  int32_t i;
  ...
  for (i = 0; i < numofDevices; i++) outTable[i] = -1;
  ostream = NULL;
...
audioSink::~audioSink(void) {
  if ((ostream != NULL) && !Pa_IsStreamStopped(ostream)) {
    ...
    while (!Pa_IsStreamStopped(ostream)) Pa_Sleep(1);
    writerRunning = false;
  }

  if (ostream != NULL) Pa_CloseStream(ostream);

  if (portAudio) Pa_Terminate();
...
bool audioSink::selectDevice(const std::string soundChannel) {
  ...
  for (i = 0; i < numofDevices; i++) {
    const std::string so = outputChannelwithRate(i, CardRate);
    if (so == std::string("")) continue;
    fprintf(stderr, "device %s seems available as %d\n", so.c_str(), i);
    if (so.find(soundChannel, 0) != std::string::npos) odev = i;
  }
  ...
  outputParameters.suggestedLatency =
      Pa_GetDeviceInfo(odev)->defaultHighOutputLatency * latency;
  ...
  err = Pa_OpenStream(&ostream, NULL, &outputParameters, CardRate, bufSize, 0,
                      this->paCallback_o, this);
...
void audioSink::restart(void) {
  ...
  if (!Pa_IsStreamStopped(ostream)) return;

  _O_Buffer->FlushRingBuffer();
  paCallbackReturn = paContinue;
  err = Pa_StartStream(ostream);
  if (err == paNoError) writerRunning = true;
}
 ...
 * 	... and the callback
 */
int audioSink::paCallback_o(const void *inputBuffer, void *outputBuffer,
  ...
  if (ud->paCallbackReturn == paContinue) {
    ...
    for (i = actualSize; i < 2 * framesPerBuffer; i++) outp[i] = 0;
  }
...
const char *audioSink::outputChannelwithRate(int16_t ch, int32_t rate) {
  ...
  if ((ch < 0) || (ch >= numofDevices)) return "";

  deviceInfo = Pa_GetDeviceInfo(ch);
  if (deviceInfo == NULL) return "";
  if (deviceInfo->maxOutputChannels <= 0) return "";

  if (OutputrateIsSupported(ch, rate)) return (deviceInfo->name);
  return "";
}

int16_t audioSink::invalidDevice(void) { return numofDevices + 128; }

bool audioSink::isValidDevice(int16_t dev) {
  return 0 <= dev && dev < numofDevices;
}

bool audioSink::selectDefaultDevice(void) { return selectDevice("default"); }

int32_t audioSink::cardRate(void) { return 48000; }

int16_t audioSink::numberofDevices(void) { return numofDevices; }
