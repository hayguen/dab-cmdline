...
static audioSink *soundOut = NULL;
...
static void bytesOut_Handler(uint8_t *data, int16_t amount, uint8_t k,
                             void *ctx) {
  (void)data;
  (void)amount;
  (void)ctx;
...
int main(int argc, char **argv) {
  ...
  try {
#ifdef HAVE_SDRPLAY
    theDevice = new sdrplayHandler(frequency, ppmCorrection, theGain, lnaState,
                                   autogain, 0, 0);
#elif HAVE_AIRSPY
    theDevice = new airspyHandler(frequency, ppmCorrection, theGain);
#elif HAVE_RTLSDR
    theDevice =
        new rtlsdrHandler(frequency, ppmCorrection, theGain, autogain, 0);
#endif
  } catch (int e) {
  ...
  //
  //	and a sound device
  theRadio = dabInit(theDevice, theMode, syncsignalHandler, systemData,
                     ensemblenameHandler, programnameHandler, fibQuality,
                     pcmHandler, dataOut_Handler, bytesOut_Handler,
                     programdataHandler, mscQuality, NULL, NULL, NULL, NULL);
  if (theRadio == NULL) {
  ...
  int timeOut = 0;
  while (!timesyncSet.load() && (++timeOut < 5)) sleep(1);
  ...
  run.store(true);
  if (serviceId != -1) programName = dab_getserviceName(theRadio, serviceId);
