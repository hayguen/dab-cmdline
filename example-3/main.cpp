...
#ifdef STREAMER_OUTPUT
static streamer *theStreamer = NULL;
#endif
...
static void pcmHandler(int16_t *buffer, int size, int rate, bool isStereo,
  ...
  if (!theStreamer->isRunning()) theStreamer->restart();
...
//	We do need an option to specify the filename
  ...
  theRadio = (void *)dabInit(theDevice, theMode, syncsignalHandler, systemData,
                             ensemblenameHandler, programnameHandler,
                             fibQuality, pcmHandler, dataOut_Handler,
                             bytesOut_Handler, programdataHandler, mscQuality,
                             NULL,  // no mot slides
  ...
  int timeOut = 0;
  while (++timeOut < waitingTime) sleep(1);
