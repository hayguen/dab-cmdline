...
int main(int argc, char **argv) {
  ...
  try {
#ifdef HAVE_SDRPLAY
    theDevice = new sdrplayHandler(frequency, ppmCorrection, theGain, 3,
                                   autogain, 0, 0);
#elif HAVE_AIRSPY
...
#elif HAVE_HACKRF
    theDevice = new hackrfHandler(frequency, ppmCorrection, lnaGain, vgaGain);
#endif
  } catch (int e) {
  ...
  while (true) {
    ...
    while (!timesyncSet.load() && (--timeSyncTime >= 0)) sleep(1);
    ...
    while (!ensembleRecognized.load() && (--freqSyncTime >= 0)) {
    ...
    print_ensembleData(outFile, theRadio, theChannel, ensembleName, ensembleId);

    print_audioheader(outFile);
    for (int i = 0; i < programNames.size(); i++) {
      if (is_audioService(theRadio, programNames[i].c_str())) {
        audiodata ad;
        dataforAudioService(theRadio, programNames[i].c_str(), &ad, 0);
        print_audioService(outFile, theRadio, programNames[i].c_str(), &ad);
        for (int j = 1; j < 5; j++) {
          packetdata pd;
          dataforDataService(theRadio, programNames[i].c_str(), &pd, j);
          if (pd.defined)
            print_dataService(outFile, theRadio, programNames[i].c_str(), j,
                              &pd);
        }
      }
    }
    ...
    for (int i = 0; i < programNames.size(); i++) {
      if (is_dataService(theRadio, programNames[i].c_str())) {
        if (firstTime) print_dataHeader(outFile);
        firstTime = false;
        for (int j = 0; j < 5; j++) {
          packetdata pd;
          dataforDataService(theRadio, programNames[i].c_str(), &pd, j);
      ...
      } else if (is_audioService(theRadio, programNames[i].c_str())) {
        for (int j = 1; j < 5; j++) {
          packetdata pd;
          dataforDataService(theRadio, programNames[i].c_str(), &pd, j);
          if (pd.defined)
