...
typedef struct {
  ...
  uint32_t
      riffSize; /* full filesize - 8 bytes (maybe with some byte missing...) */
  ...
  uint32_t Bandwidth; /* displayable BW if you want to limit the display to less
                         than Nyquist band */
  int32_t
      IQOffset; /* DC offset of the I and Q channels in 1/1000's of a count */
  int32_t Unused2;
  int32_t Unused3;
  int32_t Unused4;
  int32_t Unused5;
...
void wavePrepareHeader(unsigned samplerate, unsigned freq, int bitsPerSample,
                       int numChannels) {
  ...
  waveHdr.auxiSize = 2 * sizeof(Wind_SystemTime) +
                     9 * sizeof(int32_t); /* = 2 * 16 + 9 * 4 = 68 */
