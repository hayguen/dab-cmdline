...
//	a static variable
static extioHandler *myContext = NULL;

static int extioCallback(int cnt, int status, float IQoffs, void *IQData) {
...
extioHandler::extioHandler(QSettings *s, bool *success) {
  ...
  *success = false;
  ...
  dll_file = QFileDialog::getOpenFileName(NULL, tr("load file .."),
                                          QDir::currentPath(),
#ifdef __MINGW32__
                                          tr("libs (Extio*.dll)"));
#else
  ...
  if (dll_file == QString("")) {
    QMessageBox::warning(NULL, tr("sdr"), tr("incorrect filename\n"));
    return;
  }

#ifdef __MINGW32__
  wchars_num =
      MultiByteToWideChar(CP_UTF8, 0, dll_file.toLatin1().data(), -1, NULL, 0);
  windowsName = new wchar_t[wchars_num];
  MultiByteToWideChar(CP_UTF8, 0, dll_file.toLatin1().data(), -1, windowsName,
                      wchars_num);
...
#endif
  if (Handle == NULL) {
    QMessageBox::warning(NULL, tr("sdr"), tr("loading dll failed\n"));
    return;
  }

  if (!loadFunctions()) {
    QMessageBox::warning(NULL, tr("sdr"), tr("loading functions failed\n"));
    return;
  }
  ...
  if (!((*InitHW)(rigName, rigModel, hardwareType))) {
    QMessageBox::warning(NULL, tr("sdr"), tr("init failed\n"));
    exit(1);
  }

  SetCallback(extioCallback);
  if (!(*OpenHW)()) {
    QMessageBox::warning(NULL, tr("sdr"), tr("Opening hardware failed\n"));
    exit(1);
  }
  ...
  if (inputRate < Khz(2000) || (1000 * (inputRate / 1000) != inputRate)) {
    QMessageBox::warning(NULL, tr("sdr"), tr("cannot handle this rate"));
    return;
  }
  ...
  switch (hardwareType) {
    ...
    case exthwSCdata:
    default:
      QMessageBox::warning(NULL, tr("sdr"), tr("device not supported\n"));
      return;
...
extioHandler::~extioHandler(void) {
  ...
  if (theReader != NULL) delete theReader;
}
...
bool extioHandler::loadFunctions(void) {
  ...
  SetCallback = (pfnSetCallback)GETPROCADDRESS(Handle, "SetCallback");
  if (SetCallback == NULL) {
...
int32_t extioHandler::getRate(void) { return GetHWSR(); }
...
void extioHandler::ShowGUI(void) {
  if (L_ShowGUI != NULL) (*L_ShowGUI)();
}

void extioHandler::HideGUI(void) {
  if (L_HideGUI != NULL) (*L_HideGUI)();
}
...
int32_t extioHandler::Samples(void) {
  ...
  if (x < 0) fprintf(stderr, "toch een fout in ringbuffer\n");
  return x;
}

int32_t extioHandler::getSamples(DSPCOMPLEX *buffer, int32_t number) {
  return theBuffer->getDataFromBuffer(buffer, number);
}

int16_t extioHandler::bitDepth(void) { return theReader->bitDepth(); }

bool extioHandler::legalFrequency(int32_t f) {
  return Khz(24000) <= f && f <= Khz(2000000);
}

int32_t extioHandler::defaultFrequency(void) { return Khz(220000); }
