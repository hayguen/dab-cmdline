...
//
reader_16::reader_16(RingBuffer<DSPCOMPLEX> *p, int32_t base_16, int32_t rate)
...
int16_t reader_16::bitDepth(void) { return 16; }
//
//	The reader for 24 bit integer values
//
reader_24::reader_24(RingBuffer<DSPCOMPLEX> *p, int32_t base_24, int32_t rate)
    : virtualReader(p, rate) {
  this->base = base_24;
}
...
int16_t reader_24::bitDepth(void) { return 24; }
//
//	The reader for 32 bit integer values
//
reader_32::reader_32(RingBuffer<DSPCOMPLEX> *p, int32_t base_32, int32_t rate)
    : virtualReader(p, rate) {
  this->base = base_32;
}
...
void reader_32::processData(float IQoffs, void *data, int cnt) {
  ...
  for (i = 0; i < blockSize; i++) {
    ...
    uint8_t i2 = p[8 * i + 6];
    uint8_t i3 = p[8 * i + 7];
    int32_t re = int32_t(uint32_t(r3 << 24 | r2 << 16 | r1 << 8 | r0));
    int32_t im = int32_t(uint32_t(i3 << 24 | i2 << 16 | i1 << 8 | i0));
    IQData[i] = DSPCOMPLEX((float)re / base, (float)im / base);
  }
...
int16_t reader_32::bitDepth(void) { return 32; }
//
...
int16_t reader_float::bitDepth(void) { return 24; }
