...
typedef int (*pfnExtIOCallback)(int cnt, int status, float IQoffs,
                                void *IQdata);
...
typedef enum {
  ...
  exthwSDR14 = 1,
  exthwSDRX = 2,
  exthwUSBdata16 = 3  // the hardware does its own digitization and the audio
                      // data are returned to Winrad via the callback device.
                      // Data must be in 16-bit  (short) format, little endian.
  ,
  exthwSCdata = 4  // The audio data are returned via the (S)ound (C)ard managed
                   // by Winrad. The external hardware just controls the LO, and
                   // possibly a preselector, under DLL control.
  ,
  exthwUSBdata24 = 5  // the hardware does its own digitization and the audio
                      // data are returned to Winrad via the callback device.
                      // Data are in 24-bit  integer format, little endian.
  ,
  exthwUSBdata32 = 6  // the hardware does its own digitization and the audio
                      // data are returned to Winrad via the callback device.
                      // Data are in 32-bit  integer format, little endian.
  ,
  exthwUSBfloat32 = 7  // the hardware does its own digitization and the audio
                       // data are returned to Winrad via the callback device.
                       // Data are in 32-bit  float format, little endian.
  ,
...
typedef enum {
  ...
  // for all extIO's
  ,
  extHw_Changed_SampleRate =
      100  // sampling speed has changed in the external HW
  ,
  extHw_Changed_LO = 101  // LO frequency has changed in the external HW
  ,
  extHw_Lock_LO = 102,
  extHw_Unlock_LO = 103,
  extHw_Changed_LO_Not_TUNE =
      104  // CURRENTLY NOT YET IMPLEMENTED
           // LO freq. has changed, Winrad must keep the Tune freq. unchanged
           // (must immediately call GetHWLO() )
  ,
  extHw_Changed_TUNE =
      105  // a change of the Tune freq. is being requested.
  ...
  ,
  extHw_Mercury_DAC_ON =
      110  // enable audio output on the Mercury DAC when using the HPSDR
  ,
  extHw_Mercury_DAC_OFF =
      111  // disable audio output on the Mercury DAC when using the HPSDR
  ,
  extHw_PC_Audio_ON =
      112  // enable audio output on the PC sound card when using the HPSDR
  ,
  extHw_PC_Audio_OFF =
      113  // disable audio output on the PC sound card when using the HPSDR

  ,
  extHw_Audio_MUTE_ON =
      114  // the DLL is asking Winrad to mute the audio output
  ,
  extHw_Audio_MUTE_OFF =
      115  // the DLL is asking Winrad to unmute the audio output
  ...
  ,
  extHw_RX_SwapIQ_ON = 116  // additionaly swap IQ - this does not modify the
                            // menu point / user selection
  ,
  extHw_RX_SwapIQ_OFF =
      117  //   the user selected swapIQ is additionally applied
  ,
  extHw_TX_SwapIQ_ON = 118  // additionaly swap IQ - this does not modify the
                            // menu point / user selection
  ,
  extHw_TX_SwapIQ_OFF =
      119  //   the user selected swapIQ is additionally applied
  ...
  ,
  extHw_TX_Request =
      120  // DLL requests TX mode / User pressed PTT
           //  exciter/transmitter must wait until SetModeRxTx() is called!
  ,
  extHw_RX_Request =
      121  // DLL wants to leave TX mode / User released PTT
           //   exciter/transmitter must wait until SetModeRxTx() is called!
  ,
  ...
  ,
  extHw_PTT_as_CWkey =
      124  // handle extHw_TX_Request as extHw_CW_Pressed in CW mode
  ...
  ,
  extHw_Changed_ATTENS =
      136  // refresh selectable attenuators => starts calling GetAttenuators()
} extHWstatusT;

// codes for pfnSetModeRxTx:
typedef enum { extHw_modeRX = 0, extHw_modeTX = 1 } extHw_ModeRxTxT;
