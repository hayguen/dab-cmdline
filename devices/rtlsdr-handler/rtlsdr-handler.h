...
typedef char *(*pfnrtlsdr_set_opt_string)(rtlsdr_dev_t *dev, const char *opts,
                                          int verbose);
}
...
class rtlsdrHandler : public deviceHandler {
 public:
  rtlsdrHandler(bool openDevice, int32_t frequency, int16_t ppmCorrection,
                int16_t gain, bool autogain, uint16_t deviceIndex = 0,
                const char *deviceSerial = 0, const char *deviceOpts = 0);
  ~rtlsdrHandler(void);
  //	interface to the reader
  const char *get_opt_help(int longInfo);
 ...
 private:
  ...
  bool load_rtlFunctions(const char *libraryString);
  ...
  pfnrtlsdr_get_opt_help rtlsdr_get_opt_help;
