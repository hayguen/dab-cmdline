...
hackrfHandler::hackrfHandler(int32_t frequency, int16_t ppm, int16_t lnaGain,
                             int16_t vgaGain) {
  int err;
  int res;
  ...
  res = hackrf_set_baseband_filter_bandwidth(theDevice, 1750000);
  if (res != HACKRF_SUCCESS) {
...
void hackrfHandler::resetBuffer(void) { _I_Buffer->FlushRingBuffer(); }

int16_t hackrfHandler::bitDepth(void) { return 8; }
