...
//
rtl_tcp_client::rtl_tcp_client(std::string hostname, int32_t port,
                               int32_t frequency, int16_t gain, bool autogain,
                               int16_t ppm) {
  ...
  int res = connect(theSocket, (struct sockaddr *)&server, sizeof(server));
  if (res < 0) {
    std::cerr << "Error: " << strerror(errno) << std::endl;
    throw(23);
  }
...
int32_t rtl_tcp_client::getSamples(std::complex<float> *V, int32_t size) {
  ...
  for (i = 0; i < amount / 2; i++)
    V[i] = std::complex<float>((float(tempBuffer[2 * i] - 128)) / 128.0,
...
int16_t rtl_tcp_client::bitDepth(void) { return 8; }
