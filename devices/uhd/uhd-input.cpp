...
void uhd_streamer::run(void) {
  while (!m_stop_signal_called) {
    ...
    int32_t size1, size2;
    void *data1, *data2;
    m_theStick->theBuffer->GetRingBufferWriteRegions(10000, &data1, &size1,
                                                     &data2, &size2);
    ...
    //	      std::cerr << boost::format("Receiver error: %s") % md.strerror()
    //<< std::endl; 	      continue;
    //	   }
  }
}
...
uhdInput::uhdInput(QSettings *s, bool *success) {
  ...
  *success = false;
  ...
  try {
    ...
    std::cout << boost::format("Using Device: %s") % m_usrp->get_pp_string()
              << std::endl;
    //	set sample rate
    m_usrp->set_rx_rate(inputRate);
    inputRate = m_usrp->get_rx_rate();
    std::cout << boost::format("Actual RX Rate: %f Msps...") % (inputRate / 1e6)
              << std::endl
              << std::endl;
  ...
  connect(externalGain, SIGNAL(valueChanged(int)), this,
          SLOT(setExternalGain(int)));
...
int32_t uhdInput::getVFOFrequency(void) {
  ...
  std::cout << boost::format("Actual RX Freq: %f MHz...") % (freq / 1e6)
            << std::endl
            << std::endl;
  return freq;
}
...
bool uhdInput::restartReader(void) {
  if (m_workerHandle != 0) return true;
...
//
//	not used:
uint8_t uhdInput::myIdentity(void) { return DAB_STICK; }
...
void uhdInput::resetBuffer(void) { theBuffer->FlushRingBuffer(); }

void uhdInput::set_fCorrection(int32_t f) { (void)f; }
...
void uhdInput::setExternalGain(int32_t gain) {
  ...
  std::cout << boost::format("Actual RX Gain: %f dB...") % gain_f << std::endl
            << std::endl;
}

void uhdInput::set_KhzOffset(int32_t o) { vfoOffset = o; }

int16_t uhdInput::bitDepth(void) { return 16; }
