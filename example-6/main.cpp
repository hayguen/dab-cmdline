...
static void *theRadio = nullptr;

static tcpWriter *theWriter;

static RingBuffer<uint8_t> *buffer;

static bandHandler *theBandHandler;
...
static audioBase *soundOut = NULL;
...
static void syncsignalHandler(bool b, void *userData) {
  ...
  if (!b) theWriter->sendData(Q_TEXT_MESSAGE, std::string("no dab signal yet"));
  (void)userData;
}
//
...
int main(int argc, char **argv) {
  ...
  try {
#ifdef HAVE_SDRPLAY
    theDevice = new sdrplayHandler(frequency, my_radioData.ppmCorrection,
                                   my_radioData.theGain,
                                   3,  // lnaState
                                   my_radioData.autogain, 0, 0);
#elif HAVE_AIRSPY
    theDevice = new airspyHandler(frequency, my_radioData.ppmCorrection,
                                  my_radioData.theGain);
#elif HAVE_RTLSDR
    theDevice = new rtlsdrHandler(frequency, my_radioData.ppmCorrection,
                                  my_radioData.theGain, my_radioData.autogain);
#elif HAVE_WAVFILES
    theDevice = new wavFiles(fileName);
#elif HAVE_RTL_TCP
    theDevice =
        new rtl_tcp_client(hostname, basePort, frequency, my_radioData.theGain,
                           my_radioData.autogain, my_radioData.ppmCorrection);
#endif
  ...
  //
  soundOut =
      new audioSink(my_radioData.latency, my_radioData.soundChannel, &err);
  if (err) {
  ...
  theRadio =
      dabInit(theDevice, my_radioData.theMode, syncsignalHandler, systemData,
  ...
  if (my_radioData.autogain) theDevice->set_autogain(my_radioData.autogain);
...
void readSettings(radioData *rd) {
  ...
  if (value != std::string("")) rd->theMode = stoi(value);
  value = my_config->get_value("ALL", "Band");
  if (value != std::string("")) rd->theBand = stoi(value);
  value = my_config->get_value("ALL", "ServerPort");
  if (value != std::string("")) rd->serverPort = stoi(value);
}
...
void listener(void) {
  ...
  while (running.load()) {
    ...
    try {
      ...
      while (running.load()) {
        int amount = recv(client_sock, localBuffer, PACKET_SIZE, 0);
        if (amount <= 0) {
          throw(22);
        } else {
  ...
  while (buffer->GetRingBufferReadAvailable() >= PACKET_SIZE) {
    ...
    switch (lbuf[2]) {
      ...
      case Q_GAIN:
        ...
        my_radioData.theGain = stoi(std::string((char *)(&(lbuf[3]))));
        theDevice->setGain(my_radioData.theGain);
        break;

      case Q_SERVICE:
        my_radioData.serviceName = std::string((char *)(&(lbuf[3])));
        fprintf(stderr, "service request for %s\n",
                my_radioData.serviceName.c_str());

        if (is_audioService(theRadio, my_radioData.serviceName.c_str())) {
          audiodata ad;
          dataforAudioService(theRadio, my_radioData.serviceName.c_str(), &ad,
                              0);
          if (!ad.defined) {
      ...
      case Q_CHANNEL:
        ...
        frequency = theBandHandler->Frequency(my_radioData.theBand,
