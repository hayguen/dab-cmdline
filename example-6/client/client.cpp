...
static const char *channelTable[] = {
    "5A",  "5B",  "5C",  "5D",  "7A",  "7B",  "7C",  "7D",  "8A",
    "8B",  "8C",  "8D",  "9A",  "9B",  "9C",  "9D",  "10A", "10B",
    "10C", "10D", "11A", "11B", "11C", "11D", "12A", "12B", "12C",
    "12D", "13A", "13B", "13C", "13D", "13E", "13F", NULL};

Client::Client(QWidget *parent) : QDialog(parent) {
  int16_t i;
  setupUi(this);
  connected = false;
  connect(connectButton, SIGNAL(clicked(void)), this, SLOT(wantConnect(void)));
  connect(terminateButton, SIGNAL(clicked(void)), this, SLOT(terminate(void)));
  ...
  connect(connectionTimer, SIGNAL(timeout(void)), this, SLOT(timerTick(void)));
  connect(channelSelector, SIGNAL(activated(const QString &)), this,
          SLOT(selectChannel(const QString &)));
  connect(ensembleDisplay, SIGNAL(clicked(QModelIndex)), this,
          SLOT(selectService(QModelIndex)));
  connect(gainSlider, SIGNAL(valueChanged(int)), this, SLOT(setGain(int)));
  connect(terminateButton, SIGNAL(clicked(void)), this,
          SLOT(handle_quit(void)));
  state->setText("waiting to start");
}
//

Client::~Client(void) { connected = false; }
//
void Client::wantConnect(void) {
  ...
  connect(hostLineEdit, SIGNAL(returnPressed(void)), this,
          SLOT(setConnection(void)));
}
...
void Client::setConnection(void) {
  ...
  int32_t basePort;
  basePort = 8765;
  disconnect(hostLineEdit, SIGNAL(returnPressed(void)), this,
             SLOT(setConnection(void)));
  //
  ...
  if (!writer.waitForConnected(2000)) {
    QMessageBox::warning(this, tr("client"), tr("setting up writer failed\n"));
    return;
  }

  connected = true;
  state->setText("Connected");
  connect(&streamer, SIGNAL(readyRead(void)), this, SLOT(readData(void)));
...
void Client::selectService(QModelIndex s) {
  ...
  if (writer.state() == QAbstractSocket::ConnectedState) {
    ...
    message[1] = length;
    message[2] = Q_SERVICE;
    for (i = 0; i < length - 2; i++) message[3 + i] = ss[i];
    writer.write(message);
  }
