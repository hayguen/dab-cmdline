...
static void callback_syncSignal(bool b, void *ctx) {
  PyObject *arglist;
  PyObject *result;
...
static void callback_systemData(bool b, int16_t snr, int32_t offs, void *ctx) {
  PyObject *arglist;
  PyObject *result;
  PyGILState_STATE gstate;
...
static void callback_fibQuality(int16_t q, void *ctx) {
  PyObject *arglist;
  PyObject *result;
  PyGILState_STATE gstate;
...
static void callback_programQuality(int16_t c1, int16_t c2, int16_t c3,
                                    void *ctx) {
  PyObject *arglist;
  PyObject *result;
  PyGILState_STATE gstate;
...
static PyObject *callbackEnsembleName = NULL;
static void callback_ensembleName(std::string s, int32_t id, void *ctx) {
  PyGILState_STATE gstate;
  PyObject *arglist;
  PyObject *result;
...
static PyObject *callbackProgramName = NULL;
static void callback_programName(std::string s, int32_t id, void *ctx) {
  PyGILState_STATE gstate;
  PyObject *arglist;
  PyObject *result;
...
static void callback_audioOut(int16_t *b, int32_t size, int32_t rate,
                              void *ctx) {
  PyObject *arglist;
  PyObject *result;
  PyObject *theArray;
  ...
  gstate = PyGILState_Ensure();
  theArray = PyArray_SimpleNewFromData(2, dims, NPY_FLOAT, (void *)data);
  arglist = Py_BuildValue("Oii", theArray, size / 2, rate);
  result = PyEval_CallObject(callbackAudioOut, arglist);
  if (arglist != NULL) Py_DECREF(arglist);
  if (theArray != NULL) Py_DECREF(theArray);
  if (result != NULL) Py_DECREF(result);
  PyGILState_Release(gstate);
}
...
static void callback_dataOut(std::string str, void *ctx) {
  PyObject *arglist;
  PyObject *result;
  PyGILState_STATE gstate;
...
static void callback_programdata(audiodata *d, void *ctx) {
  PyObject *arglist;
  PyObject *result;
  PyGILState_STATE gstate;
  ...
  if (!d->defined) return;
  gstate = PyGILState_Ensure();
  arglist = Py_BuildValue("hhhhh", d->subchId, d->startAddr, d->length,
                          d->bitRate, d->ASCTy);
...
PyObject *dabInit_p(PyObject *self, PyObject *args) {
  ...
  (void)PyArg_ParseTuple(args, "shhOOOOOOOOO", &theChannel, &theGain, &theMode,
                         &cbsH, &cbsD, &cbeN, &cbpN, &cbfQ, &cbaO, &cbdO, &cbpD,
                         &cbpQ);
  //
  ...
  if (!PyCallable_Check(cbpN)) {
    PyErr_SetString(PyExc_TypeError,
                    "parameter for program data must be callable");
    goto err;
  }
  ...
  Py_XINCREF(cbsH);
  if (callbackSyncSignal != NULL) Py_XDECREF(callbackSyncSignal);
  callbackSyncSignal = cbsH;

  Py_XINCREF(cbsD);
  if (callbackSystemData != NULL) Py_XDECREF(callbackSystemData);
  callbackSystemData = cbsD;

  Py_XINCREF(cbeN);
  if (callbackEnsembleName != NULL) Py_XDECREF(callbackEnsembleName);
  callbackEnsembleName = cbeN;

  Py_XINCREF(cbpN);
  if (callbackProgramName != NULL) Py_XDECREF(callbackProgramName);
  callbackProgramName = cbpN;

  Py_XINCREF(cbfQ);
  if (callbackFibQuality != NULL) Py_XDECREF(callbackFibQuality);
  callbackFibQuality = cbfQ;

  Py_XINCREF(cbaO);
  if (callbackAudioOut != NULL) Py_XDECREF(callbackAudioOut);
  callbackAudioOut = cbaO;

  Py_XINCREF(cbdO);
  if (callbackDataOut != NULL) Py_XDECREF(callbackDataOut);
  callbackDataOut = cbdO;

  Py_XINCREF(cbpD);
  if (callbackProgramData != NULL) Py_XDECREF(callbackProgramData);
  callbackProgramData = cbpD;

  Py_XINCREF(cbpQ);
  if (callbackProgramQuality != NULL) Py_XDECREF(callbackProgramQuality);
  callbackProgramQuality = cbpQ;

  frequency = dabBand.Frequency(BAND_III, theChannel);
  try {
#ifdef HAVE_SDRPLAY
    theDevice = new sdrplayHandler(frequency, 3, theGain, true, 0, 0);
#elif HAVE_AIRSPY
    theDevice = new airspyHandler(frequency, 0, theGain);
#elif HAVE_RTLSDR
  ...
  result = dabInit(
      ...
      (syncsignal_t)&callback_syncSignal, (systemdata_t)&callback_systemData,
      (ensemblename_t)&callback_ensembleName,
      (programname_t)&callback_programName, (fib_quality_t)&callback_fibQuality,
      (audioOut_t)&callback_audioOut, (dataOut_t)&callback_dataOut,
      (bytesOut_t)NULL, (programdata_t)&callback_programdata,
...
static struct PyModuleDef dab_lib = {PyModuleDef_HEAD_INIT,
                                     "dab_lib",
                                     "",
                                     -1,
                                     module_methods,
