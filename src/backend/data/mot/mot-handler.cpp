...
motHandler::motHandler(motdata_t motdataHandler, void *ctx) {
  this->motdataHandler = motdataHandler;
  this->ctx = ctx;
  orderNumber = 0;

  theDirectory = nullptr;
  for (int i = 0; i < 15; i++) motTable[i].orderNumber = -1;
}
...
motHandler::~motHandler(void) {
  ...
  for (i = 0; i < 15; i++)
    if (motTable[i].orderNumber > 0) delete motTable[i].motSlide;
  if (theDirectory != nullptr) delete theDirectory;
}
...
void motHandler::add_mscDatagroup(std::vector<uint8_t> msc) {
  ...
  if (crcFlag && !check_CRC_bits(data, msc.size())) return;

  if (extensionFlag) next += 16;
  ...
  switch (groupType) {
    ...
    case 6:
      if (segmentNumber == 0) {  // MOT directory
        ...
        //                  segment [10];
        theDirectory =
            new motDirectory(motdataHandler, ctx, transportId, segmentSize,
        ...
        theDirectory->directorySegment(transportId, &motVector[2],
                                       segmentNumber, segmentSize, lastFlag);
      }
      break;
...
motObject *motHandler::getHandle(uint16_t transportId) {
  ...
  if (theDirectory != nullptr) return theDirectory->getHandle(transportId);
  return nullptr;
}
