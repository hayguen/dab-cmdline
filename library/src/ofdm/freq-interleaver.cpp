...
void interLeaver::createMapper(int16_t T_u, int16_t V1, int16_t lwb,
  ...
  tmp[0] = 0;
  for (i = 1; i < T_u; i++) tmp[i] = (13 * tmp[i - 1] + V1) % T_u;
  for (i = 0; i < T_u; i++) {
    if (tmp[i] == T_u / 2) continue;
    if ((tmp[i] < lwb) || (tmp[i] > upb)) continue;
...
interLeaver::interLeaver(uint8_t dabMode) : params(dabMode) {
  permTable = new int16_t[params.get_T_u()];
  switch (dabMode) {
    case 1:
    default:  // shouldn't happen
      createMapper(params.get_T_u(), 511, 256, 256 + params.get_carriers(),
                   permTable);
      break;

    case 2:
      createMapper(params.get_T_u(), 127, 64, 64 + params.get_carriers(),
                   permTable);
      break;

    case 3:
      createMapper(params.get_T_u(), 63, 32, 32 + params.get_carriers(),
                   permTable);
      break;

    case 4:
      createMapper(params.get_T_u(), 255, 128, 128 + params.get_carriers(),
                   permTable);
      break;
  }
}
//
//
interLeaver::~interLeaver(void) { delete permTable; }
//
