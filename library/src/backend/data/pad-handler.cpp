 ...
 */
padHandler::padHandler(dataOut_t dataOut, motdata_t motdata_Handler,
                       void *ctx) {
...
padHandler::~padHandler(void) {
  if (currentSlide != nullptr) delete currentSlide;
}
...
void padHandler::handle_shortPAD(uint8_t *b, int16_t last, uint8_t CIf) {
  ...
  if (CIf) {  // Has CI
    ...
    switch (AcTy) {
      ...
      case 2:  // start of new fragment, extract the length
        if (firstSegment && !lastSegment) {
          ...
          if (dynamicLabelText.size() > 0) dataOut(dynamicLabelText, ctx);
          dynamicLabelText.clear();
        }
      ...
      case 3:  // continuation of fragment
        ...
        if ((still_to_go <= 0) && (shortpadData.size() > 1)) {
          ...
          if (UnicodeUcs2 == (CharacterSet)charSet)
            fprintf(stderr,
                    "warning: ignoring pad text for dynamicLabel cause "
                    "of unimplemented Ucs2 conversion\n");
          else {
            std::string segmentText = toStringUsingCharset(
                (const char *)(shortpadData.data()), (CharacterSet)charSet,
                shortpadData.size());
    ...
    if ((still_to_go <= 0) && (shortpadData.size() > 0)) {
      ...
      if (UnicodeUcs2 == (CharacterSet)charSet)
        fprintf(stderr,
                "warning: ignoring pad text for dynamicLabel cause of "
                "unimplemented Ucs2 conversion\n");
      else {
        std::string segmentText =
            toStringUsingCharset((const char *)(shortpadData.data()),
                                 (CharacterSet)charSet, shortpadData.size());
        dynamicLabelText.append(segmentText);
      }
      shortpadData.resize(0);
      if (dynamicLabelText.length() > 0) dataOut(dynamicLabelText, ctx);
      dynamicLabelText.clear();
    }
  }
}
...
void padHandler::handle_variablePAD(uint8_t *b, int16_t last, uint8_t CI_flag) {
  ...
  if (CI_flag == 0) {
    if (mscGroupElement && (xpadLength > 0)) {
      ...
      for (j = 0; j < xpadLength; j++) data[j] = b[last - j];
  ...
  if (mscGroupElement) {
    xpadLength = 0;
    for (i = 0; i < CI_Index; i++) xpadLength += lengthTable[CI_table[i] >> 5];
  ...
  for (i = 0; i < CI_Index; i++) {
    ...
    data.resize(length);
    for (j = 0; j < length; j++) data[j] = b[base - j];
...
void padHandler::build_MSC_segment(std::vector<uint8_t> data) {
  ...
  //	is
  int32_t size =
      (int)data.size() < dataGroupLength ? data.size() : dataGroupLength;
  ...
  if ((groupType != 3) && (groupType != 4)) return;  // do not know yet
  ...
  switch (groupType) {
    case 3:
      if (currentSlide == nullptr) {
        ...
        currentSlide =
            new motObject(motdata_Handler, false, transportId, &data[index + 2],
                          segmentSize, lastFlag, ctx);
      } else {
        if (currentSlide->get_transportId() == transportId) break;
      ...
      break;
    case 4:
      if (currentSlide == nullptr) return;
