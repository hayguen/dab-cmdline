...
dataProcessor::dataProcessor(int16_t bitRate, packetdata *pd,
                             bytesOut_t bytesOut, motdata_t motdataHandler,
                             void *ctx) {
...
dataProcessor::~dataProcessor(void) { delete my_dataHandler; }
...
void dataProcessor::handlePacket(uint8_t *data) {
  ...
  if (packetState == 0) {   // waiting for a start
    if (firstLast == 02) {  // first packet
      ...
      for (i = 0; i < series.size(); i++) series[i] = data[24 + i];
...
void dataProcessor::handleTDCAsyncstream(uint8_t *data, int16_t length) {
  ...
  (void)command;
  (void)usefulLength;
  if (!check_CRC_bits(data, packetLength * 8)) return;
}
