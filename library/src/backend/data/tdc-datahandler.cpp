...
tdc_dataHandler::tdc_dataHandler(int16_t appType, bytesOut_t bytesOut,
                                 void *ctx) {
...
void tdc_dataHandler::add_mscDatagroup(std::vector<uint8_t> m) {
  ...
  while (offset < size) {
    ...
    for (i = 0; i < 4; i++) checkVector[i] = getBits(data, offset + i * 8, 8);
    //
    ...
    checkVector[5 + length + 1] = getBits(data, offset + 5 * 8, 8);
...
int32_t tdc_dataHandler::handleFrame_type_0(uint8_t *data, int32_t offset,
  ...
  for (i = 0; i < length; i++) buffer[i] = getBits(data, offset + i * 8, 8);

  if (bytesOut != nullptr) bytesOut(buffer, length, 0, ctx);
...
int32_t tdc_dataHandler::handleFrame_type_1(uint8_t *data, int32_t offset,
  ...
  if (bytesOut != nullptr) bytesOut(buffer, length, 1, ctx);
  return offset + length * 8;
}
