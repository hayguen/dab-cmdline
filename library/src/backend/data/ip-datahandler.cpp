...
ip_dataHandler::ip_dataHandler(RadioInterface *mr, bool show_crcErrors) {
  connect(this, SIGNAL(writeDatagram(char *, int)), mr,
          SLOT(sendDatagram(char *, int)));
  this->show_crcErrors = show_crcErrors;
  this->crcErrors = 0;
  this->handledPackets = 0;
  connect(this, SIGNAL(show_ipErrors(int)), mr, SLOT(show_ipErrors(int)));
}
...
void ip_dataHandler::add_mscDatagroup(QByteArray &msc) {
  ...
  if (ipLength < msc.size() / 8) {  // just to be sure
    ...
    for (i = 0; i < ipLength; i++) ipVector[i] = getBits_8(data, next + 8 * i);
    if ((ipVector[0] >> 4) != 4) return;  // should be version 4
