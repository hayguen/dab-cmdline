...
const EPG::gl EPG::genre_list[] = {
    ...
    {"6.10.1", "No strobing"},
    {"6.10.2",
     "Strobing that could impact on those suffering from "
     "Photosensitive epilepsy"},
    {"6.10.2.1",
     "One scene of strobing that could impact on those suffering "
     "from photosensitive epilepsy"},
    {"6.10.2.2",
     "Occasional strobing that could impact on those suffering "
     "from photosensitive epilepsy"},
    {"6.10.2.3",
     "Frequent strobing that could impact on those suffering from "
     "photosensitive epilepsy"},
    {"6.2", "NUDITY"},
    ...
    {"6.3.3.1",
     "One scene of infliction of strong psychological or physical "
     "pain to human beings"},
    {"6.3.3.2",
     "Occasional scenes of infliction of strong psychological or "
     "physical pain to human beings"},
    {"6.3.3.3",
     "Frequent scenes of infliction of strong psychological or "
     "physical pain to human beings"},
    ...
    {"6.5.2",
     "Deliberate infliction of pain to fantasy characters (including "
     "animation)"},
    {"6.5.2.1",
     "One scene of deliberate infliction of pain to fantasy "
     "characters (including animation)"},
    {"6.5.2.2",
     "Occasional deliberate infliction of pain to fantasy "
     "characters (including animation)"},
    {"6.5.2.3",
     "Frequent deliberate infliction of pain to fantasy characters "
     "(including animation)"},
    {"6.5.3", "Deliberate killing of fantasy characters (including animation)"},
    {"6.5.3.1",
     "One scene of deliberate killing of fantasy characters "
     "(including animation)"},
    {"6.5.3.2",
     "Occasional deliberate killing of fantasy characters "
     "(including animation)"},
    {"6.5.3.3",
    ...
    {"6.7.2",
     "Factual material that may cause distress, including verbal "
     "descriptions of traumatic events and the telling of sensitive "
     "human interest stories."},
    {"6.7.3",
     "Mild scenes of blood and gore (including medical procedures, injuries "
     "from accidents, terrorists attack, murder, disaster, war)"},
...
QDomDocument EPG::getFile(const QDate &date, uint32_t sid, bool bAdvanced) {
  CDateAndTime d;
  ...
  epg.doc.documentElement().insertBefore(
      epg.doc.createComment(fileName), epg.doc.documentElement().firstChild());
  return epg.doc;
}
  ...
  while (!l1.isNull()) {
    if (l1.nodeName() == "programme") {
      ...
      if (min_time <= start && start <= max_time) {
        QMap<shortCRIDType, ProgrammeType>::ConstIterator existing =
            progs.find(start);
...
void EPG::saveChannels(const QString &fileName) {
  ...
  for (map<uint32_t, CServiceInformation>::const_iterator i =
           Parameters.ServiceInformation.begin();
    ...
    for (set<string>::const_iterator j = si.label.begin(); j != si.label.end();
         j++) {
    ...
    if (n.nodeName() == "service") {
      ...
      while (!e.isNull()) {
        if (e.isElement()) {
          ...
          if (s.tagName() == "shortName") name = s.text().toUtf8().constData();
          if (s.tagName() == "serviceID") sid = s.attribute("id", "0");
        }
        e = e.nextSibling();
      }
      if (name != "") {
...
QString EPG::toHTML(void) const {
  ...
  r << "<html><body><table>";
  for (QMap<shortCRIDType, ProgrammeType>::const_iterator i = progs.begin();
       i != progs.end(); i++) {
...
time_t EPGTime::parseTime(const QString &time) {
  ...
  if (time.length() == 0) return t;  // invalid
...
int EPGTime::parseDuration(const QString &duration) {
  if (duration == "") return 0;  // invalid

  if (duration[0] != 'P') return 0;  // invalid

  QRegExp r("PT((\\d+)H)?((\\d+)M)?((\\d+)S)?");
  if (r.indexIn(duration) == -1) return 0;

  QStringList dur = r.capturedTexts();
  int h = 0, m = 0;
  if (dur[2] != "") h = dur[2].toInt();
  if (dur[4] != "") m = dur[4].toInt();
  return 60 * h + m;
}
...
void EPGTime::augment(const EPGTime &t) {
  if (t.time != 0) time = t.time;
  if (t.actualTime != 0) actualTime = t.actualTime;
  if (t.duration != 0) duration = t.duration;
  if (t.actualDuration != 0) actualDuration = t.actualDuration;
}
...
void EPGBearer::augment(const EPGBearer &b) {
  if (b.trigger != 0) trigger = b.trigger;
  if (b.id != "") id = b.id;
}
...
void LocationType::augment(const LocationType &l) {
  if ((time.size() == 1) && (l.time.size() == 1)) {
    ...
    for (size_t i = 0; i < l.time.size(); i++) time.push_back(l.time[i]);
  }
  ...
  if ((bearer.size() == 1) && (l.bearer.size() == 1)) {
    ...
    for (size_t i = 0; i < l.bearer.size(); i++) bearer.push_back(l.bearer[i]);
  }
}
...
void GenreType::augment(const GenreType &g) {
  if (href == "") href = g.href;
  if (name.text == "") name = g.name;
  if (definition.text == "") definition = g.definition;
  if (mimeValue == "") mimeValue = g.mimeValue;
  if (g.preferred) preferred = true;
  if (type == undef) type = g.type;
}
...
void ProgrammeType::augment(const ProgrammeType &p) {
  if ((location.size() == 0)) {
    ...
    if (p.location.size() > 0) location[0].augment(p.location[0]);
    // TODO
  }
  ...
  if (p.recommendation == 0) recommendation = true;
  if (p.broadcast == off_air) broadcast = p.broadcast;
  if (bitrate == 0) bitrate = p.bitrate;
  if (CA == "") CA = p.CA;
  if (genre.size() == 0) {
    genre = p.genre;
  } else {
    if (p.genre.size() > 0) genre[0].augment(p.genre[0]);
    // TODO
  }
  ...
  if (link.size() == 0) {
    ...
    for (size_t i = 0; i < p.link.size(); i++) link.push_back(p.link[i]);
  }
