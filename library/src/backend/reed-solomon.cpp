...
reedSolomon::reedSolomon(uint16_t symsize, uint16_t gfpoly, uint16_t fcr,
                         uint16_t prim, uint16_t nroots)
  ...
  for (i = 0, root = fcr * prim; i < nroots; i++, root += 1) {
    ...
    for (j = i; j > 0; j--) {
      if (generator[j] != 0) {
        uint16_t p1 =
            myGalois.multiply_power(myGalois.poly2power(generator[j]), root);
        generator[j] =
            myGalois.add_poly(generator[j - 1], myGalois.power2poly(p1));
    ...
    generator[0] = myGalois.power2poly(
        myGalois.multiply_power(root, myGalois.poly2power(generator[0])));
  }
  for (i = 0; i <= nroots; i++)
    generator[i] = myGalois.poly2power(generator[i]);
}

reedSolomon::~reedSolomon(void) { delete generator; }
    ...
    if (feedback != codeLength) { /* feedback term is non-zero */
      for (j = 1; j < nroots; j++)
        bb[j] = myGalois.add_poly(bb[j],
                                  myGalois.power2poly(myGalois.multiply_power(
                                      feedback, generator[nroots - j])));
    }
    /*	Shift */
    memmove(&bb[0], &bb[1], sizeof(bb[0]) * (nroots - 1));
    if (feedback != codeLength)
      bb[nroots - 1] =
          myGalois.power2poly(myGalois.multiply_power(feedback, generator[0]));
    else
      bb[nroots - 1] = 0;
  }
}
...
void reedSolomon::enc(const uint8_t *r, uint8_t *d, int16_t cutlen) {
  ...
  for (i = cutlen; i < codeLength; i++) rf[i] = r[i - cutlen];

  encode_rs(rf, bb);
  for (i = cutlen; i < codeLength - nroots; i++) d[i - cutlen] = rf[i];
  //	and the parity bytes
  for (i = 0; i < nroots; i++) d[codeLength - cutlen - nroots + i] = bb[i];
}
...
int16_t reedSolomon::decode_rs(uint8_t *data) {
  ...
  if (computeSyndromes(data, syndromes)) return 0;
  ...
  for (j = rootCount - 1; j >= 0; j--) {
    ...
    for (i = omega_degree; i >= 0; i--) {
      if (omega[i] != codeLength) {
        uint16_t tmp = myGalois.multiply_power(
            omega[i], myGalois.pow_power(i, rootTable[j]));
        num1 = myGalois.add_poly(num1, myGalois.power2poly(tmp));
      }
    }
    uint16_t tmp = myGalois.multiply_power(
        myGalois.pow_power(rootTable[j], myGalois.divide_power(fcr, 1)),
        codeLength);
    ...
    for (i = min(lambda_degree, nroots - 1) & ~1; i >= 0; i -= 2) {
      if (Lambda[i + 1] != codeLength) {
        uint16_t tmp = myGalois.multiply_power(
            Lambda[i + 1], myGalois.pow_power(i, rootTable[j]));
    ...
    if (num1 != 0) {
      if (locTable[j] >= uint8_t(codeLength - nroots))
        ...
        uint16_t tmp2 = myGalois.multiply_power(myGalois.poly2power(num1),
...
uint8_t reedSolomon::getSyndrome(uint8_t *data, uint8_t root) {
  ...
  for (j = 1; j < codeLength; j++) {
    if (syn == 0)
      ...
      uint16_t uu1 =
          myGalois.pow_power(myGalois.multiply_power(fcr, root), prim);
      syn = myGalois.add_poly(
          data[j], myGalois.power2poly(
                       myGalois.multiply_power(myGalois.poly2power(syn), uu1)));
      //	                                                   (fcr + root)
      //* prim)));
    }
  }
...
uint16_t reedSolomon::computeLambda(uint8_t *syndromes, uint8_t *Lambda) {
  ...
  for (i = 0; i < nroots; i++) Corrector[i] = Lambda[i] = 0;
  ...
  while (K <= nroots) {
    ...
    //	Compute new lambda
    for (i = 0; i < nroots; i++)
      Lambda[i] = myGalois.add_poly(
          Lambda[i], myGalois.multiply_poly(error, Corrector[i]));
    if ((2 * L < K) && (error != 0)) {
      L = K - L;
      for (i = 0; i < nroots; i++)
    ...
    for (i = nroots - 1; i >= 1; i--) Corrector[i] = Corrector[i - 1];
    Corrector[0] = 0;

    //	and compute a new error
    error = syndromes[K];
    for (i = 1; i <= K; i++) {
      error = myGalois.add_poly(
          error, myGalois.multiply_poly(syndromes[K - i], Lambda[i]));
    }
    K += 1;
...
int16_t reedSolomon::computeErrors(uint8_t *Lambda, uint16_t deg_lambda,
                                   uint8_t *rootTable, uint8_t *locTable) {
  int16_t i, j, k;
  int16_t rootCount = 0;
  //
  ...
  for (i = 1, k = iprim - 1; i <= codeLength; i++, k = (k + iprim)) {
    ...
    for (j = deg_lambda; j > 0; j--) {
      if (workRegister[j] != codeLength) {
        ...
        result =
            myGalois.add_poly(result, myGalois.power2poly(workRegister[j]));
      }
    }
    if (result != 0)  // no root
      continue;
  ...
  if (rootCount != deg_lambda) return -1;
  return rootCount;
}
 ...
 */
uint16_t reedSolomon::computeOmega(uint8_t *syndromes, uint8_t *lambda,
                                   uint16_t deg_lambda, uint8_t *omega) {
  int16_t i, j;
  int16_t deg_omega = 0;
  ...
  for (i = 0; i < nroots; i++) {
    ...
    for (; j >= 0; j--) {
      if ((myGalois.poly2power(syndromes[i - j]) != codeLength) &&
          (lambda[j] != codeLength)) {
        uint16_t res = myGalois.power2poly(myGalois.multiply_power(
            myGalois.poly2power(syndromes[i - j]), lambda[j]));
