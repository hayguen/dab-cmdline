...
void Client::setConnection(void) {
  ...
  if (!streamer.waitForConnected(2000)) {
    QMessageBox::warning(this, tr("client"), tr("setting up stream failed\n"));
    return;
  }
...
void Client::readData(void) {
  ...
  while (streamer.bytesAvailable() > 4 * 512) {
    ...
    for (i = 0; i < d.size(); i++) handle((uint8_t)(d[i]));
  }
}
...
void Client::handleFrameType_1(uint8_t *dataBuffer, int16_t Length) {
  ...
  if (dataBuffer[3] == 0) {  // encryption 0
    ...
    while (index < Length) {
      ...
      uint16_t segLen = (dataBuffer[index + 1] << 8) | dataBuffer[index + 2];
      uint16_t crc = (dataBuffer[index + 3] << 8) | dataBuffer[index + 4];
      uint8_t testVector[18];
      testVector[0] = sCi;
      testVector[1] = dataBuffer[index + 1];
      testVector[2] = dataBuffer[index + 2];
      for (i = 0; i < 13; i++) testVector[i + 3] = dataBuffer[index + i + 5];
      testVector[16] = dataBuffer[index + 3];
      testVector[17] = dataBuffer[index + 4];
      if (usCalculCRC(testVector, 16) == crc)
        fprintf(stderr, "sCi = %o, segLen = %d (index = %d, Length = %d)\n",
...
bool Client::serviceComponentFrameheaderCRC(uint8_t *data, int16_t offset,
                                            int16_t maxL) {
  uint8_t testVector[18];
  int16_t i;
