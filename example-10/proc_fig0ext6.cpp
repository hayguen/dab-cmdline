...
struct LinkingInfoVal {
  ...
  // uint8_t IdLQ;  // 2 bit: 0: DAB SIDs, 1: RDS PIs, 2: reserved, 3: DRM/AMSS
  // SID
...
static int linkInfoWrite() {
  ...
  if (linkInfo_strA.length() <= 0 || linkInfo_strB.length() <= 0) return retIdx;

  for (int k = 0; k < 16; ++k) {
    if (!lastLinkInfoValid[k]) continue;
  ...
  for (int k = 0; k < 16; ++k) {
    if (k != wrIdx && lastLinkInfoAge[k] < 65536) ++lastLinkInfoAge[k];
  }
  return retIdx;
}
...
void flush_fig0_ext6() {
  ...
  for (const auto &n : aLinkInfoMap[currLinkInfoMapIdx]) {
    ...
    fprintf(stderr, "\nLink %d: %s, %s link, LSN: 0x%X\n", linkNo,
            (LA ? "  Active" : "Inactive"), (SH ? "Hard" : "Soft"), LSN);
    ++linkNo;
    if (lnkVal.ID[0].size()) {
      fprintf(stderr, "  DAB SID=0x%X, %s: ", lnkVal.SID[0],
              (lnkVal.ILS[0] ? "Intl" : "Nat."));
      for (const auto &k : lnkVal.ID[0]) fprintf(stderr, "0x%X, ", k.first);
      fprintf(stderr, "\n");
    }
    if (lnkVal.ID[1].size()) {
      fprintf(stderr, "  PS  SID=0x%X, %s: ", lnkVal.SID[1],
              (lnkVal.ILS[1] ? "Intl" : "Nat."));
      for (const auto &k : lnkVal.ID[1]) fprintf(stderr, "0x%X, ", k.first);
      fprintf(stderr, "\n");
    }
    if (lnkVal.ID[2].size()) {
      fprintf(stderr, "  AM  SID=0x%X, %s: ", lnkVal.SID[2],
              (lnkVal.ILS[2] ? "Intl" : "Nat."));
      for (const auto &k : lnkVal.ID[2]) fprintf(stderr, "0x%X, ", k.first);
      fprintf(stderr, "\n");
    }
  }

  // uint8_t IdLQ;  // 2 bit: 0: DAB SIDs, 1: RDS PIs, 2: reserved, 3: DRM/AMSS
  // SID uint32_t ID[3][16];  // 0: DAB, 1: PS, 2: AM uint32_t SID[3];     //
  // DAB_SID for .. uint32_t num[3];     // # of elements in ID[3][16] uint8_t
  // ILS[3]; uint8_t PD[3];    // 1 bit uint8_t SHD[3];    // 1 bit

  aLinkInfoMap[currLinkInfoMapIdx].clear();
}

void proc_fig0_ext6(const uint8_t *figData, int figLen,
                    unsigned fibCallbackNo) {
  if (initLinkInfo) initLinkInfoVars();
  ...
  if (CN == 0)  // new DB_STRT?
  {
    ...
    if (createdIdx >= 0 && linkInfo_gotFromBegin) {
      ...
      fprintf(stderr, "\nSERVICE LINKING INFO #%d: %s, FIB#%u:%u..%u\n%s\n%s\n",
              numDifferenServiceLinkInfos,
              lastLinkInfos[createdIdx].first.c_str(), lastLinkfibNoCount,
              lastLinkfibNoStart, lastLinkfibNoEnd,
              lastLinkInfos[createdIdx].second.c_str(), linkInfo_strC.c_str());
#else
      fprintf(stderr, "\nSERVICE LINKING INFO #%d: %s, FIB#%u:%u..%u\n%s\n",
              numDifferenServiceLinkInfos,
              lastLinkInfos[createdIdx].first.c_str(), lastLinkfibNoCount,
  ...
  std::swap(linkInfo_ostrA, tmpE1);
  linkInfo_ostrA << "OE " << (OE ? "OTHR" : "THIS") << " Ensemble, "
                 << (PD ? "32/data" : "16/program");
  ...
  while (figLen >= 2) {
    ...
    if (lastLinkfibNoEnd != fibCallbackNo) ++lastLinkfibNoCount;
    ...
    linkInfo_ostrC << "FIB#" << fibCallbackNo << ": CN " << (int)CN << ", OE "
                   << (int)OE << ", PD " << (int)PD << ", idList "
                   << (int)id_list_flag << ", LA " << (int)LA << ", SH "
                   << (int)SH << ", ILS " << (int)ILS << ", numIds "
                   << (int)num_ids << "\n";
#endif

    if (id_list_flag) {
      const uint8_t id_list_usage = getMax8Bits(4, 16, figData);
      const uint8_t Rfu = (id_list_usage)&8;  // must be 0. else definition of
                                              // IdLQ and ID List is invalid
      ...
      if (Rfu == 0 && IdLQ != 2) {
        ...
        switch (IdLQ) {
          ...
          case 2:
            ...
            break;
          case 3:
            pacIdLQ = "DRM/AMSS SID";
            break;
        }

        const uint32_t lnkKey =
            uint32_t(LSN) | (uint32_t(LA) << 16) | (uint32_t(SH) << 24);
        ...
        if (PD == 1) {
          ...
          linkInfo_ostrB << "  SHD-" << std::dec << (int)SHD << ", " << pacIdLQ
                         << ", ECC/ID 8/16-Bit:" << (num_ids ? " " : "\n");
          for (uint8_t k = 0U; k < num_ids; ++k) {
            uint16_t ecc = getMax8Bits(8, 24 + k * 24, figData);
            uint16_t id = getMax16Bits(16, 24 + k * 24 + 8, figData);
            linkInfo_ostrB << "0x" << std::hex << std::uppercase
                           << (unsigned)ecc << "/0x" << std::hex << (unsigned)id
                           << ((k == 0 && linkInfo_first_is_SID) ? " (=DAB_SID)"
                                                                 : "")
                           << (k + 1 == num_ids ? "\n" : ", ");
            if (linkInfo_first_is_SID)
